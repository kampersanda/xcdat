...
public:
  ...
  // Returns the IDs of keys included as prefixes of a given key. The IDs are
  // appended to 'ids' and the number is returned. By using 'limit', you can
  // restrict the maximum number of returned IDs.
  size_t common_prefix_lookup(const uint8_t* key, size_t length,
                              std::vector<id_type>& ids,
                              size_t limit = std::numeric_limits<size_t>::max()) const;

  // Returns the IDs of keys starting with a given key. The IDs are appended to
  // 'ids' and the number is returned. By using 'limit', you can restrict the
...
private:
  ...
  bool prefix_match_(const uint8_t* key, size_t length,
                     size_t pos, size_t tail_pos) const {
    assert(pos < length);
